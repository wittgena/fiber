...
class WasmExecutor:
    ...
    async def execute_stream(self, context: TaskContext) -> AsyncGenerator[Contract, None]:
        log.info(f"[{SOURCE_NAME}] Dispatching task '{context.task_type}' (ID: {context.task_id}) to WASM Kernel")
        ...
        while True:
            ...
            ## 1. Handle WASM execution errors
            ...
            ## 3. Emit intermediate state transitions
            yield Contract(
                id=generate_topos_id(),
