# anchor.phase.ingress.gateway.reflect
## @lineage: bound.ingress.gateway.reflect
## @lineage: xphi.proxy.gateway
import httpx
import uvicorn
from fastapi import FastAPI, Request, HTTPException, Depends
from fastapi.responses import StreamingResponse, Response

from anchor.registry.proxy.setting import gateway_settings, pypi_settings
from anchor.phase.reflect.server.auth.token.introspect import IntrospectionTokenVerifier
...
# 브로커의 introspect 엔드포인트를 바라보는 검증기 인스턴스 초기화
...
def main():
    log.info(f"Starting Brane Reflect Gateway on {gateway_settings.host}:{gateway_settings.port}")
    uvicorn.run(
        "xphi.proxy.gateway:app",
