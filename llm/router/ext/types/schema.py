# fiber.llm.router.ext.types.schema
## @lineage: fiber.dphi.model.ext.types.schema
...
## @lineage: eco.bound.agent.adapter.types.schema
...
from fiber.llm.router.mapper.pydantic import CoreSchema
from fiber.llm.router.util import SAMPLE_TEXT, truncate_text

if TYPE_CHECKING:
    from haystack.schema import Document as HaystackDocument
    from semantic_kernel.memory.memory_record import MemoryRecord
    from fiber.llm.router.ext.llm.model.types.block import BaseContentBlock
...
class ImageNode(TextNode):
    ...
    def get_content_blocks(self, metadata_mode: MetadataMode = MetadataMode.NONE) -> list[BaseContentBlock]:
        from fiber.llm.router.ext.llm.model.types.block import ImageBlock
        blocks: list[BaseContentBlock] = []
        blocks.extend(self.get_metadata_content_blocks(metadata_mode))
...
class IndexNode(TextNode):
    ...
    def _serialize_obj(self) -> Any:
        from fiber.llm.router.ext.storage.docstore.utils import doc_to_json
        try:
            if self.obj is None:
                return None
    ...
    def from_dict(cls, data: Dict[str, Any], **kwargs: Any) -> Self:  # type: ignore
        ...
        if isinstance(obj, str):
            ...
            from fiber.llm.router.ext.storage.docstore.utils import json_to_doc
            try:
