# fiber.llm.router.ext.llm.handle.gemini
...
from fiber.llm.router.mapper.pydantic import BaseModel, ValidationError
...
from fiber.llm.router.ext.llm.model.flex import _repair_incomplete_json
        ...
        if finish_reason != types.FinishReason.STOP:
            ...
            try:
                if hasattr(candidate, "model_dump"):
                    ...
                    candidate_dump = {"raw_content": str(candidate)}
                
                def safe_serializer(obj):
                    if isinstance(obj, bytes):
                        return f"<bytes object: len={len(obj)}>"
                    return str(obj)
                    
                dump_str = json.dumps(candidate_dump, ensure_ascii=False, indent=2, default=safe_serializer)
                
                logger.error(f"[Gemini Debug] Abnormal finish_reason: {reason}\n[Payload Dump]:\n{dump_str}")
                error_details = f"\n\n--- [Model Payload Dump] ---\n{dump_str}\n----------------------------"
            except Exception as e:
                logger.error(f"[Gemini Debug] Failed to extract candidate dump: {str(e)}")
                error_details = f"\n\n[Model Payload Dump Failed: {str(e)}]\nRaw Object: {str(candidate)}"

            raise RuntimeError(f"Response was terminated early: {reason}{error_details}")
...
async def prepare_chat_params(
    ...
    **kwargs: Any,
    ...
    try:
        def _safe_dump(obj):
            if hasattr(obj, "model_dump"): return obj.model_dump(exclude_none=True)
            if hasattr(obj, "__dict__"): return obj.__dict__
            return str(obj)
            
        final_tools = config.get("tools")
        if final_tools:
            dump_str = json.dumps(final_tools, ensure_ascii=False, indent=2, default=_safe_dump)
            logger.debug(f"Tools 구조:\n{dump_str}")
    except Exception as e:
