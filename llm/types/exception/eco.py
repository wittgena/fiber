# fiber.llm.types.exception.eco
## @lineage: fiber.llm.exception.eco
from __future__ import annotations
from typing import Any, Dict, Optional, Union
from enum import Enum
import httpx
import openai
from fiber.llm.types.exception.types import LLMContextWindowExceedError, LLMMalformedConversationHistoryError
...
class FiberCommonStrings(Enum):
    redacted_by_fiber = "redacted by fiber. 'fiber.turn_off_message_logging=True'"
...
def _ensure_mock_response(response: Optional[httpx.Response], status_code: int) -> httpx.Response:
    ...
    return httpx.Response(
        ...
        request=httpx.Request(method="POST", url="https://api.fiber.local/v1"),
    )
...
def _ensure_mock_request(request: Optional[httpx.Request]) -> httpx.Request:
    if request is not None and isinstance(request, httpx.Request):
        return request
    return httpx.Request(method="POST", url="https://api.fiber.local/v1")

class FiberExceptionMixin:
    def init_fiber_attrs(
    ...
    def __str__(self) -> str:
        ...
        if getattr(self, "num_retries", None):
            _msg += f" | Fiber Retried: {self.num_retries} times"
...
class BaseLLMException(Exception, FiberExceptionMixin):
    def __init__(
        ...
        self.body = body
        self.init_fiber_attrs(message, None, None)
        Exception.__init__(self, self.message)

class AuthenticationError(openai.AuthenticationError, FiberExceptionMixin):
...
class NotFoundError(openai.NotFoundError, FiberExceptionMixin):
...
class BadRequestError(openai.BadRequestError, FiberExceptionMixin):
...
class PermissionDeniedError(openai.PermissionDeniedError, FiberExceptionMixin): # type: ignore
...
class RateLimitError(openai.RateLimitError, FiberExceptionMixin): # type: ignore
    def __init__(self, message, llm_provider, model, response=None, debug_info=None, max_retries=None, num_retries=None):
...
class MockException(openai.APIError):
    def __init__(
        ...
        self.status_code = status_code
        self.message = "fiber.MockException: {}".format(message)
        self.llm_provider = llm_provider
        self.model = model
        self.debug_info = debug_info
        self.max_retries = max_retries
...
class FiberUnknownProvider(BadRequestError):
    def __init__(self, model: str, custom_llm_provider: Optional[str] = None):
        self.message = FiberCommonStrings.llm_provider_not_provided.value.format(
...
class MidStreamFallbackError(ServiceUnavailableError):  # type: ignore
    def __init__(
        ...
        self.message = f"fiber.MidStreamFallbackError: {message}"
        self.model = model
        self.llm_provider = llm_provider
    ...
    def __str__(self):
        ...
        if self.num_retries:
            _message += f" Fiber Retried: {self.num_retries} times"
        if self.max_retries:
            _message += f", Fiber Max Retries: {self.max_retries}"
