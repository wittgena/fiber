# dphi.eco.conv.state
## @lineage: engine.atoa.context.state
import os
import json
import inspect
...
from ator.state.event.llm.observation import (
    AgentErrorEvent,
    ObservationEvent,
    UserRejectObservation,
)
from engine.parser.conv.event import Event
from engine.parser.conv.event import EventID
from ator.topos.space.base import BaseWorkspace
from ator.state.context.status import ConverStatus
from engine.parser.conv.types import ConversationCallbackType, ConversationID, ConversationTags
    ...
    def __setattr__(self, name, value):
        ...
        if old is _sentinel or old != value:
            ...
            if callback is not None and old is not _sentinel:
                try:
                    from ator.state.event.conv import ConversationStateUpdateEvent
