# dphi.space.terminal.tmux.pool
## @lineage: agent.space.terminal.tmux.pool
## @lineage: bound.space.terminal.tmux.pool
## @lineage: agent.runtime.terminal.tmux.pool
## @lineage: ator.runtime.terminal.tmux.pool
## @lineage: agent.bridge.terminal.tmux.pool
## @lineage: agent.space.tool.terminal.tmux.pool
## @lineage: agent.tool.terminal.tmux.pool
## @lineage: ator.driver.tool.terminal.tmux.pool
## @lineage: driver.tool.terminal.tmux.pool
## @lineage: ator.bound.tool.terminal.tmux.pool
## @lineage: eco.tool.terminal.tmux.pool
## @lineage: engine.tool.terminal.tmux.pool
...
from fiber.dphi.space.terminal.session.builder import sanitized_env
...
from fiber.dphi.space.terminal.tmux.interface import TmuxTerminal
...
class TmuxPanePool:
    ...
    def initialize(self) -> None:
        ...
        session_name = f"surgent-pool-{self.username}-{uuid.uuid4()}"
    ...
    def close(self) -> None:
        ...
        try:
            ...
            log.warning(f"Error killing pool session: {e}")
    ...
    def _create_pane(self) -> PooledTmuxTerminal:
        ...
        log.debug(f"Created pooled pane #{len(self._all_panes)}: {active_pane}")
        return terminal
    ...
    def checkin(self, terminal: PooledTmuxTerminal) -> None:
        ...
        with self._lock:
            if terminal not in self._all_panes:
                log.warning("Attempted to checkin a pane not from this pool")
                return
            if not self._closed:
                self._available.append(terminal)

        self._semaphore.release()
        log.debug(f"Checked in pane: {terminal.pane}")
    ...
    def replace(self, old_terminal: PooledTmuxTerminal) -> PooledTmuxTerminal:
        ...
        try:
            ...
            log.debug(f"Error killing replaced pane window: {e}")
        old_terminal._closed = True

        log.debug(f"Replaced pane {old_pane_id} -> {new_pane_id}")
        return new_terminal
