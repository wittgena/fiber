# fiber.dev.ex.agent.conv.confirm
## @lineage: theoria.phase.model.conv.security.confirm
...
class ConfirmPolicy(DiscMixin, ABC):
    @abstractmethod
...
class AlwaysConfirm(ConfirmPolicy):
    def should_confirm(
        self,
...
class NeverConfirm(ConfirmPolicy):
    def should_confirm(self, risk: SecurityRisk = SecurityRisk.UNKNOWN) -> bool:
        return False

class ConfirmRisky(ConfirmPolicy):
