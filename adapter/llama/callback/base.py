...
from adapter.llama.callback.base_handler import BaseCallbackHandler
from adapter.llama.callback.schema import (
    BASE_TRACE_EVENT,
    LEAF_EVENTS,
    CBEventType,
    EventPayload,
...
class CallbackManager(BaseCallbackHandler, ABC):
    def __init__(self, handlers: Optional[List[BaseCallbackHandler]] = None):
        ...
        if len(handlers) == 0:
            from adapter.llama.anchor.bound.settings import Settings
