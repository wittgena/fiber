# atoa.disc.memory.codon.registry
...
class AtorDefinition(BaseModel):
    ...
    def get_confirmation_policy(self) -> ConfirmationPolicyBase | None:
        ...
        if self.permission_mode == "always_confirm":
            from eco.gov.atoa.security.confirm import AlwaysConfirm
            return AlwaysConfirm()
        if self.permission_mode == "never_confirm":
            from eco.gov.atoa.security.confirm import NeverConfirm
            return NeverConfirm()
        if self.permission_mode == "confirm_risky":
            from eco.gov.atoa.security.confirm import ConfirmRisky
            return ConfirmRisky()
