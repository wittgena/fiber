...
class AgentStateSnapshot:
    """
    @desc: Gov 환경으로부터 전달받은 페이로드를 추상화한 읽기 전용 상태 객체.
    """
    def __init__(self, task_payload: dict):
        ...
        # [핵심 수정 1] dict를 다시 알맞은 Event Pydantic 객체로 복원합니다.
        raw_events = task_payload.get("events", [])
        self.events = [self._parse_event(e) for e in raw_events]

    def _parse_event(self, data: dict) -> Event:
        """Heuristic parser to restore polymorphic events from dicts"""
        ...
        elif "action" in data and "tool_name" in data:
...
class Activator(Ator):
    ...
    async def process_task(self, task_payload: dict, tunnel: UniversalFacade, response_topic: str) -> None:
        ...
        if snapshot.iteration == 0:
            ...
            # [핵심 수정 2] Emit 하기 전에 현재 핸들러들이 바라볼 메모리(snapshot.events)에도 적재합니다.
        ...
        async def async_on_event(event: Event):
            # 생성된 이벤트를 로컬 스냅샷 메모리에도 반영하여 연쇄 핸들링 보장
    ...
    async def _emit_event_to_gov(self, event: Event, tunnel: UniversalFacade, response_topic: str) -> None:
        from arch.topos.bound.payload import StreamPayloadAdapter # 앞서 만든 어댑터 사용
        ...
        if isinstance(event, ActionEvent):
            payload_raw = {
                ...
                "action_args": event.action.parameters,
