...
from atoa.agent.conv.chat import select_chat_options
...
class DriverIO:
    ...
    def responses(
        ...
        **kwargs,
        ...
        """
        [Adapter] Legacy Responses API 호출을 Completion API로 전환합니다.
        기존 시스템과의 호환성을 위해 인터페이스는 유지하되, 내부적으로는 self.completion을 호출합니다.
        """
        if include or store:
            log.debug(
        ...
        # 자체적인 복잡한 포매팅 로직을 모두 버리고, 
        # 순수 Chat Completion 규격으로 안전하게 위임합니다.
