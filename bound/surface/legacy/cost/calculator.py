# bound.surface.legacy.cost.calculator
## @lineage: bound.adapter.surface.legacy.cost.calculator
## @lineage: bound.surface.cost.calculator
import time
...
from bound.surface.legacy.cost.builtin import BuiltInToolCostTracker
from bound.surface.token.counter import token_counter
from bound.proxy.eco.cost.unit.calc import UnitCostCalculator
from bound.proxy.eco.cost.policy import CostPolicy
from bound.surface.legacy.cost.parser import (
...
from bound.surface.legacy.types import CallTypesLiteral, StandardBuiltInToolsParams, Usage, CostPerToken
from bound.surface.legacy.param.rerank import RerankBilledUnits
from bound.surface.resolver.provider import ProviderResolver
from watcher.plane.emitter import get_emitter

log = get_emitter("cost.calculator")
LitellmLoggingObject = Any

_global_cost_policy = CostPolicy()
...
def cost_per_token(
    ...
    response_cost = _global_cost_policy.calculate_custom_pricing(
...
def completion_cost(
    ...
    try:
        ...
        prompt_characters, completion_characters = None, None
        cache_creation_input_tokens, cache_read_input_tokens = None, None
        audio_transcription_file_duration: float = 0.0
        ...
        selected_model = ProviderResolver.resolve_selected_model(
            model=model,
        ...
        potential_model_names = [selected_model, ProviderResolver.resolve_response_model(completion_response)]
        ...
        for idx, current_model in enumerate(potential_model_names):
            try:
                if completion_response is not None and isinstance(completion_response, (BaseModel, dict)):
                    ...
                    if isinstance(usage_obj, BaseModel) and not UsageTelemetryParser.is_known_usage_object(usage_obj):
                    ...
                    if hidden_params is not None:
                        ...
                        if service_tier is None:
                            ...
                            if raw_traffic_type:
                                service_tier = ProviderResolver.map_traffic_to_tier(raw_traffic_type)
                else:
                ...
                if call_type in _IMAGE_RESPONSE_CALL_TYPES and UsageTelemetryParser.is_image_response(completion_response):
                    ...
                    audio_transcription_file_duration = UsageTelemetryParser.extract_transcription_duration(completion_response)
                
                elif call_type in _RERANK_CALL_TYPES:
                    units, tokens = UsageTelemetryParser.extract_rerank_units(completion_response)
                    if units is not None:
                        rerank_billed_units = units
                        completion_tokens = tokens
                
                elif call_type == _AREALTIME_CALL_TYPE and UsageTelemetryParser.is_realtime_stream(completion_response):
                ...
                # [REFACTORED] 동적 리졸버가 적용된 Additional Costs 로직
                additional_costs = _global_cost_policy.get_additional_costs(
                ...
                # [REFACTORED] 단일 파이프라인으로 할인과 마진 동시 적용
                breakdown = _global_cost_policy.apply_cost_modifiers(
                    base_cost=_final_cost, 
                    custom_llm_provider=custom_llm_provider
                )
                
                if breakdown.modifiers:
                    log.debug(f"Cost modifiers breakdown: {[m.model_dump() for m in breakdown.modifiers]}")
...
def response_cost_calculator(
    response_object: AnyResponseObject,
    model: str,
    ...
    try:
        ...
        if isinstance(response_object, BaseModel) and hasattr(response_object, "_hidden_params"):
            ...
            provider_response_cost = CostPolicy.extract_cost_from_headers(response_object._hidden_params)
