# bound.surface.bridge.channel.convert.request
## @lineage: bound.channel.convert.request
## @lineage: bound.bridge.channel.convert.request
## @lineage: bound.channel.bridge.convert.request
...
from bound.surface.bridge.channel.param.format import type_to_response_format_param
...
class RequestBuilder:
    ...
    def preprocess_requested_params(cls, params: Dict[str, Any]) -> ResponsesAPIOptionalRequestParams:
        ...
        from bound.surface.bridge.channel.param.optional import PreProcessNonDefaultParams
        ...
        if "previous_response_id" in non_default:
            non_default["previous_response_id"] = ResponseIdentityManager.decode_previous_response_id_to_original_previous_response_id(non_default["previous_response_id"])
        ...
        for item in request_input:
            if isinstance(item, dict):
                ...
                if enc_content and isinstance(enc_content, str):
                    _, unwrapped = ResponseIdentityManager._unwrap_encrypted_content_with_model_id(enc_content)
...
class IdentityMutator:
    ...
    def inject_model_identity(cls, response: Union[ResponsesAPIResponse, Dict[str, Any]], custom_llm_provider: Optional[str], litellm_metadata: Optional[Dict[str, Any]] = None) -> Union[ResponsesAPIResponse, Dict[str, Any]]:
        ...
        updated_id = ResponseIdentityManager._build_responses_api_response_id(model_id=model_id, custom_llm_provider=custom_llm_provider, response_id=resp_id)
    ...
    def _update_encrypted_content(cls, response: Union["ResponsesAPIResponse", Dict[str, Any]], model_id: Optional[str]) -> None:
        ...
        for item in output:
            ...
            if enc_content and isinstance(enc_content, str):
                _set_val(item, "encrypted_content", ResponseIdentityManager._wrap_encrypted_content_with_model_id(enc_content, model_id))
                if item_id and isinstance(item_id, str):
                    _set_val(item, "id", ResponseIdentityManager._build_encrypted_item_id(model_id, item_id))
    ...
    def encode_item(cls, item: Any, custom_llm_provider: Optional[str], model_id: Optional[str]) -> None:
        ...
        def _maybe_encode(cid: str) -> Optional[str]:
            if ResponseIdentityManager._decode_container_id(cid).get("custom_llm_provider") is not None:
                return None
            return ResponseIdentityManager._build_container_id(custom_llm_provider=custom_llm_provider, model_id=model_id, container_id=cid)
