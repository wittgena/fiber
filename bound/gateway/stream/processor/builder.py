# bound.gateway.stream.processor.builder
## @lineage: gateway.stream.processor.builder
## @lineage: bound.stream.processor.builder
## @lineage: bound.transport.stream.processor.builder
## @lineage: bound.surface.stream.processor.builder
...
class ChunkBuilderProcessor:
    ...
    def build_base_response(self, chunks: List[Dict[str, Any]]) -> ModelResponse:
        ...
        id = ChunkBuilderProcessor._get_chunk_id(chunks)
        object = chunk["object"]
        created = chunk["created"]
        first_chunk_model = chunk["model"]
        # Get the actual model - for Azure Model Router, this finds the real model from later chunks
        model = ChunkBuilderProcessor._get_model_from_chunks(chunks, first_chunk_model)
