# bound.gateway.stream.processor
## @lineage: bound.gateway.stream.processor.chunk
import json
import logging
import time
import traceback
...
from eco.legacy.types import Delta, GenericStreamingChunk as GChunk, ChatCompletionMessageToolCall, Function, FunctionCall

from eco.legacy.switch.params import ModelResponse, ModelResponseStream, StreamingChoices, Usage
...
def preserve_upstream_non_openai_attributes(model_response: ModelResponseStream, original_chunk: Any) -> None:
...
class StreamChunkProcessor:
    ...
    정제하고, 단일 패스(Single-pass)로 최종 완성된 ModelResponse를 점진적으로 조립(Incremental Build)합니다.
    """
    ...
    def __init__(
        ...
        self._second_to_last_content = None
        ...
        # ✅ [ADD] Incremental Build를 위한 최종 응답(Final Response) 객체 초기화
        ...
        self.final_response._hidden_params = self._base_hidden_params.copy()
        
        # Tool Calls 누적을 위한 맵
        self.tool_calls_map = {}
        self.function_call_accum = {"name": "", "arguments": ""}
    ...
    def process_raw_chunk(self, chunk: Any) -> Optional[ModelResponseStream]:
        ...
        try:
            ...
            # ✅ [ADD] 파싱이 완료된 청크를 최종 응답 객체에 실시간으로 병합
            if processed_chunk is not None:
                self._accumulate_to_final_response(processed_chunk)
    ...
    def _accumulate_to_final_response(self, chunk: ModelResponseStream):
        """청크의 데이터를 final_response에 실시간으로 병합(Incremental Build)합니다."""
        if not self.final_response.id and chunk.id:
            self.final_response.id = chunk.id
        
        if not getattr(self.final_response, "system_fingerprint", None) and chunk.system_fingerprint:
            self.final_response.system_fingerprint = chunk.system_fingerprint
        ...
        if delta.content:
            ...
            # 반복 감지를 위한 로컬 변수 업데이트
            self._second_to_last_content = self._last_content
            self._last_content = delta.content
        ...
        if getattr(delta, "tool_calls", None):
            for tc in delta.tool_calls:
                ...
                if getattr(tc.function, "arguments", None):
                    self.tool_calls_map[idx]["function"]["arguments"] += tc.function.arguments

            # 맵을 ChatCompletionMessageToolCall 리스트로 변환하여 갱신
            formatted_tcs = []
        ...
        # 3. 구형 Function Call 누적 (Deprecated)
        if getattr(delta, "function_call", None):
            if getattr(delta.function_call, "name", None):
                self.function_call_accum["name"] = delta.function_call.name
            if getattr(delta.function_call, "arguments", None):
                self.function_call_accum["arguments"] += delta.function_call.arguments
            
            message.function_call = FunctionCall(**self.function_call_accum)

        # 4. Usage 누적 (보통 스트림 마지막 청크에 포함됨)
        if hasattr(chunk, "usage") and chunk.usage:
            self.final_response.usage = chunk.usage

    def get_complete_response(self) -> ModelResponse:
        """스트림 종료 시 호출되어, 조립이 완료된 단일 ModelResponse 객체를 반환합니다."""
        self.final_response.choices[0].finish_reason = self.received_finish_reason or "stop"
        
        # Tool Call이 발생했는데 텍스트가 없는 경우 content를 None으로 맞춰주는 OpenAI 표준 처리
        if self.final_response.choices[0].message.content == "" and getattr(self.final_response.choices[0].message, "tool_calls", None):
    ...
    def return_processed_chunk_logic(self, completion_obj: Dict[str, Any], model_response: ModelResponseStream, response_obj: Dict[str, Any]) -> Optional[ModelResponseStream]:
        ...
        if is_chunk_non_empty:
            ...
            if self.sent_last_chunk is True:
                ...
                # [REMOVED] self.chunks.append(model_response)
                return model_response
    ...
    def raise_on_model_repetition(self) -> None:
        # 리스트 조회가 아닌 로컬 상태 변수로 체크
        if self._last_content is None or not isinstance(self._last_content, str) or len(self._last_content) <= 2:
            self._repeated_messages_count = 1
            return

        if self._second_to_last_content == self._last_content:
            self._repeated_messages_count += 1
        else:
            self._repeated_messages_count = 1
        ...
        if self._repeated_messages_count >= limit:
            raise config.InternalServerError(
                message=f"The model is repeating the same chunk = {self._last_content}.",
                model="",
                llm_provider="",
            )
