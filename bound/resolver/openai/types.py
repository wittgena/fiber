...
from arch.topos.bound.surge.model import DynamicSurgeModel
from bound.mapper.param.response import GenericResponseOutputItem, OutputCodeInterpreterCall, OutputFunctionToolCall, OutputImageGenerationCall
...
class OutputTokensDetails(DynamicSurgeModel):
...
class InputTokensDetails(DynamicSurgeModel):
...
class ResponseAPIUsage(DynamicSurgeModel):
    input_tokens: int
...
class ResponsesAPIResponse(DynamicSurgeModel):
    id: str
    created_at: int
...
class ReasoningSummaryTextDeltaEvent(DynamicSurgeModel):
...
class ReasoningSummaryTextDoneEvent(DynamicSurgeModel):
...
class ReasoningSummaryPartDoneEvent(DynamicSurgeModel):
    type: Literal[ResponsesAPIStreamEvents.REASONING_SUMMARY_PART_DONE]
    item_id: str
    output_index: int
    sequence_number: int
    summary_index: int = 0
    part: DynamicSurgeModel


class OutputItemAddedEvent(DynamicSurgeModel):
    type: Literal[ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED]
    output_index: int
    item: Optional[DynamicSurgeModel]


class OutputItemDoneEvent(DynamicSurgeModel):
    type: Literal[ResponsesAPIStreamEvents.OUTPUT_ITEM_DONE]
    output_index: int
    sequence_number: int = 1
    item: DynamicSurgeModel
...
class ContentPartAddedEvent(DynamicSurgeModel):
...
class ContentPartDonePartOutputText(DynamicSurgeModel):
    type: Literal["output_text"]
    text: str
    annotations: List[DynamicSurgeModel]
    logprobs: Optional[List[OpenAIChatCompletionLogprobsContent]]


class ContentPartDonePartRefusal(DynamicSurgeModel):
    type: Literal["refusal"]
    refusal: str


class ContentPartDonePartReasoningText(DynamicSurgeModel):
...
class ContentPartDoneEvent(DynamicSurgeModel):
...
class OutputTextDeltaEvent(DynamicSurgeModel):
...
class OutputTextAnnotationAddedEvent(DynamicSurgeModel):
...
class OutputTextDoneEvent(DynamicSurgeModel):
...
class RefusalDeltaEvent(DynamicSurgeModel):
...
class RefusalDoneEvent(DynamicSurgeModel):
...
class FunctionCallArgumentsDeltaEvent(DynamicSurgeModel):
...
class FunctionCallArgumentsDoneEvent(DynamicSurgeModel):
...
class MCPListToolsInProgressEvent(DynamicSurgeModel):
...
class MCPListToolsCompletedEvent(DynamicSurgeModel):
...
class MCPListToolsFailedEvent(DynamicSurgeModel):
...
# MCP Call Events
class MCPCallInProgressEvent(DynamicSurgeModel):
...
class MCPCallArgumentsDeltaEvent(DynamicSurgeModel):
...
class MCPCallArgumentsDoneEvent(DynamicSurgeModel):
...
class MCPCallCompletedEvent(DynamicSurgeModel):
...
class MCPCallFailedEvent(DynamicSurgeModel):
...
class ImageGenerationPartialImageEvent(DynamicSurgeModel):
    type: Literal[ResponsesAPIStreamEvents.IMAGE_GENERATION_PARTIAL_IMAGE]
    partial_image_index: int
    b64_json: str


class ErrorEventError(DynamicSurgeModel):
...
class ErrorEvent(DynamicSurgeModel):
    type: Literal[ResponsesAPIStreamEvents.ERROR]
    sequence_number: int
    error: ErrorEventError


class GenericEvent(DynamicSurgeModel):
    type: str
...
class OpenAIModerationResult(DynamicSurgeModel):
...
class OpenAIModerationResponse(DynamicSurgeModel):
    """
