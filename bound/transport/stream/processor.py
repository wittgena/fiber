# bound.transport.stream.processor
import json
import logging
import time
import traceback
...
from bound.transport.stream.chunk.parser import StreamChunkParser
...
class StreamChunkProcessor:
    ...
    # =========================================================================
    # 1. Core Processing Method (핵심 조립 메서드)
    # =========================================================================
    ...
    def return_processed_chunk_logic(self, completion_obj: Dict[str, Any], model_response: ModelResponseStream, response_obj: Dict[str, Any]) -> Optional[ModelResponseStream]:
        ...
        if is_chunk_non_empty:
            ...
            if self.sent_last_chunk is True:
                ...
                # ✅ FIX 2: StopIteration 예외 던지기 제거 -> Wrapper 루프 자연 종료 유도
    ...
    def raise_on_model_repetition(self) -> None:
        ...
        # 안전한 속성 조회: config에 해당 값이 없으면 기본값(100)으로 대체
