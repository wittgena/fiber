# bound.transport.acp.conn.base
## @lineage: bound.transport.conn.base
...
from bound.transport.acp.conn.worker import DefaultMessageDispatcher, MessageDispatcher, RpcTaskKind, NotificationRunner, RequestRunner
from bound.transport.acp.conn.sender import MessageSender, SenderFactory
from bound.transport.acp.conn.queue import MessageQueue, RpcTask, InMemoryMessageQueue
from bound.watcher.plane.topos import span_context
...
class Connection:
    ...
    async def _run_request(self, message: dict[str, Any]) -> Any:
        ...
        # [작동 원리] 
        # OTel이나 Logfire로 향하던 span이 이제 내부 flow_scope(phase="acp.request")로 열립니다.
        # attributes에 담긴 method 정보 역시 LogEvent의 컨텍스트로 안전하게 상속됩니다.
        with span_context(
            "acp.request",
    ...
    async def _run_notification(self, message: dict[str, Any]) -> None:
        ...
        # [작동 원리] OTel span이 내부 flow_scope(phase="acp.notification")로 완벽히 전환됩니다.
