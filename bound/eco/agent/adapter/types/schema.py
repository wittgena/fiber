# bound.eco.agent.adapter.types.schema
...
from bound.eco.agent.adapter.pydantic import CoreSchema
from bound.eco.agent.adapter.parser import SAMPLE_TEXT, truncate_text

if TYPE_CHECKING:
    from haystack.schema import Document as HaystackDocument
    from semantic_kernel.memory.memory_record import MemoryRecord
    from bound.eco.agent.llm.model.types.block import BaseContentBlock
...
class BaseNode(BaseComponent):
    ...
    def get_metadata_content_blocks(
        ...
        from bound.eco.agent.llm.model.types.block import TextBlock
...
class ImageNode(TextNode):
    ...
    def get_content_blocks(self, metadata_mode: MetadataMode = MetadataMode.NONE) -> list[BaseContentBlock]:
        from bound.eco.agent.llm.model.types.block import ImageBlock
        blocks: list[BaseContentBlock] = []
        blocks.extend(self.get_metadata_content_blocks(metadata_mode))
...
class IndexNode(TextNode):
    ...
    def _serialize_obj(self) -> Any:
        from bound.eco.agent.adapter.storage.docstore.utils import doc_to_json
        try:
            if self.obj is None:
                return None
    ...
    def from_dict(cls, data: Dict[str, Any], **kwargs: Any) -> Self:  # type: ignore
        ...
        if isinstance(obj, str):
            ...
            from bound.eco.agent.adapter.storage.docstore.utils import json_to_doc
            try:
