# bound.agent.llm.model.types.block
...
from bound.agent.adapter.constants import DEFAULT_CONTEXT_WINDOW, DEFAULT_NUM_OUTPUTS
from bound.agent.adapter.types.schema import ImageDocument
from bound.agent.adapter.parser import get_tokenizer, resolve_binary
...
class BaseContentBlock(ABC, BaseModel):
    ...
    def get_template_vars(self) -> list[str]:
        ...
        """
        from bound.agent.adapter.parser import get_template_vars
    ...
    def format_vars(self, **kwargs: Any) -> "BaseContentBlock":
        ...
        For that to work, the validation on those fields would need to be updated though.
        """
        from bound.agent.adapter.parser import format_string
...
class TextBlock(BaseContentBlock):
    ...
    async def asplit(
        ...
        from bound.agent.adapter.node_parser import TokenTextSplitter
