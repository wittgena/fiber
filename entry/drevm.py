...
from receptor.surface.adapter.shadow import ShadowAdapter
...
class MockOrchestrator:
    ...
    async def fetch_account_state(self, address: str, storage_slots: List[str] = None) -> Dict[str, Any]:
        ...
        return MockNetBuilder.evm_state_snapshot(address, is_contract=is_contract, should_revert=is_revert)

    async def fetch_block_context(self) -> Dict[str, Any]:
        return MockNetBuilder.evm_block_context()
...
class EvmShadowWorkflow(Workflow):
    def __init__(self, target_contract: str, user_intent: Dict[str, Any], rpc_url: Optional[str] = None, use_mock: bool = True):
        ...
        self.notary_keys = [node["priv"] for node in MockNotarySwarm(size=3).notaries]
    ...
    async def phase_simulation(self, msg: EvmPreparedMsg) -> WorkflowMessage:
        ...
        try:
            ...
            parsed_out = json.loads(result.output)
            self.log.info(f"  └─ [PASS] Execution Successful via drevm.wasm. (Gas: {parsed_out.get('gas_used')})")
            return EvmExecutedMsg()
...
class DrevmPipelineCLI:
    ...
    async def execute_scenario(self, name: str, mode: str, revert: bool, scenario_type: str = "ERC20_TRANSFER") -> bool:
        ...
        if self.args.calldata == "0x":
            intent = MockNetBuilder.evm_user_intent(scenario_type=scenario_type, should_revert=revert)
    ...
    @classmethod
    def run_cli(cls):
        parser = argparse.ArgumentParser(description="DREVM Shadow Execution Tester")
