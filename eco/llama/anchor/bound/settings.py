# eco.llama.anchor.bound.settings
## @lineage: adapter.llama.anchor.bound.settings
## @lineage: llama.anchor.bound.settings
## @lineage: anchor.inter.bound.settings
## @lineage: bound.adapter.llama.settings
from dataclasses import dataclass
from typing import Any, Callable, List, Optional

from bound.gateway.token.splitter import TokenSplitter
from bound.gateway.token.window import ContextWindow
from bound.gateway.token.counter import token_counter
from bound.resolver.model.cost import model_cost

from eco.llama.anchor.bound.prompts.utils import is_chat_model
from eco.llama.anchor.bound.base.embeddings.base import BaseEmbedding
from eco.llama.anchor.bound.schema import TransformComponent
from eco.llama.anchor.bound.types import PydanticProgramMode

from eco.llama.callback.base import BaseCallbackHandler, CallbackManager
from eco.llama.flow.embedding.utils import EmbedType, resolve_embed_model
from eco.llama.flow.llm.llm import LLM
from eco.llama.flow.llm.utils import LLMType, resolve_llm
...
class _Settings:
    ...
    def context_window_engine(self) -> ContextWindow:
        ...
        if self._context_window is None:
            from bound.gateway.token.encoder import encode, decode
            safe_model = getattr(self.llm, "model", "gpt-3.5-turbo") if self._llm else "gpt-3.5-turbo"
