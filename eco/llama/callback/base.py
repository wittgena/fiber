# eco.llama.callback.base
## @lineage: adapter.llama.callback.base
## @lineage: llama.callback.base
## @lineage: xor.loop.callback.base
## @lineage: xphi.loop.callback.base
import logging
import uuid
from abc import ABC
...
from eco.llama.callback.base_handler import BaseCallbackHandler
from eco.llama.callback.schema import (
    BASE_TRACE_EVENT,
    LEAF_EVENTS,
    CBEventType,
    EventPayload,
...
class CallbackManager(BaseCallbackHandler, ABC):
    def __init__(self, handlers: Optional[List[BaseCallbackHandler]] = None):
        ...
        if len(handlers) == 0:
            from eco.llama.anchor.bound.settings import Settings
