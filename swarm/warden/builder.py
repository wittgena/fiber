# swarm.warden.builder
## @lineage: topos.warden.builder
...
from swarm.warden.client.pypi import PypiMCPClient
from swarm.warden.cred import CredentialStore, ClientCredentialsOAuthProvider
...
class SecurityWarden:
    """@desc: Manages local fail-fast policies and automated proxy authentication"""
    def __init__(self, registry_url: str):
        self.registry_url = registry_url
        
        # [EVOLUTION] Localized minimal intelligence.
        # Deep vulnerability scanning is fully delegated to the Proxy Membrane and WASM.
        self.intelligence = {
            "warden_policies": {
                "allowed_hosts": ["localhost", "127.0.0.1", "nexus.next-phase.com"]
            },
            "quarantine_targets": [] # Handled by the upstream proxy logic now
        }
    ...
    def inject_auth(self, env: dict) -> dict:
        ...
        # Strict boundary check before routing traffic
        if registry_domain not in allowed_hosts:
            log.critical(f"🚨 Target registry ({registry_domain}) is not a whitelisted membrane!")
            sys.exit(1)
    ...
    def audit_lockfile(self, lock_file: Path):
        log.info(f"🛡️ Validating {lock_file.name} structure locally...")
        quarantine_targets = [
        ...
        with open(lock_file, "r") as f:
            ...
            for bad_pkg in quarantine_targets:
                if bad_pkg and bad_pkg in lock_content:
                    log.critical(f"🚨 Pre-flight Violation: Local blacklist triggered for ('{bad_pkg}').")
                    sys.exit(1)
                    
        log.info("   ✅ Local integrity verified. Heavy security audits delegated to the Proxy Membrane.")
...
class BuildEngine:
    ...
    def install_packages(self, lock_file: Path, mode: BuildMode):
        log.info("📦 Installing packages via authorized Proxy Membrane...")
        if mode == BuildMode.MIGRATE_UVLOCK:
            self._run(["uv", "sync", "--no-dev"])
        else:
            self._run(["uv", "pip", "install", "-r", lock_file, "--require-hashes", "--no-deps"])
        log.info("   🎉 Installation complete. Dependencies sealed.")
...
if __name__ == "__main__":
    ...
    log.info(f"[SecureBuilder] Starting in '{build_mode.name}' mode (Vulnerability intelligence delegated to Kernel).")
