# fiber.phase.flow.agent.launcher
## @lineage: surgent.flow.agent.launcher
...
class LauncherWorkflow(Workflow):
    ...
    def __init__(self, name: str, run_context: dict, **kwargs):
        ...
        self.exchange_adapter = ClearingAdapter(clearing_house_pub_key="local_clearing_pub_key")
        self.transition = FlowTransition(origin=name)
        self.topology_spec = run_context.get("topology_spec", DEFAULT_TOPOS_SPEC)
        
        self._quorum_target = len([
            node_id for node_id, spec in self.topology_spec.items() 
            if spec.get("type") in ["executor", "runtime"] or "Config" in node_id
        ])
    ...
    def mount_topology(self):
        RuntimeController.assemble_and_mount(
            topos=self, run_context=self.run_context, 
            broker=self.broker, topology_spec=self.topology_spec
        )
        return self
    ...
    async def evaluate_quorum(self, msg: AgentConfiguredMessage) -> None:
        ...
        if self._config_count >= self._quorum_target:  
            log.info(f"[{self.name}] 🚦 Quorum ({self._quorum_target}) reached. Signaling Workspace creation wave.")
            self._workspace_deployed = True
    ...
    async def ignite_agent_loop(self, msg: WorkspaceReadyMessage) -> None:
        entry_node_name = self.run_context.get("entry_point", "ConfigPolicyNode")
        log.info(f"[{self.name}] ⚙️ Workspace Ready (Ref: {msg.workspace_ref}). Igniting Entry Agent ({entry_node_name})...")
        
        dispatched = False
        for child in self.children:
            if child.name == entry_node_name:
                # 💡 분리된 ToposController를 통해 Blueprint 페이로드 주입 (의존성 역전 없이 순방향 실행)
                RuntimeController.dispatch_payload(
                    policy_node=child, 
        ...
        if not dispatched:
            log.error(f"[{self.name}] 🚨 Entry node '{entry_node_name}' not found in topology! Cannot ignite workflow.")
    ...
    async def settle_and_terminate(self, msg: TaskConvergedMessage) -> None:
        ...
        receipt = await RuntimeController.settle_dominium(
    ...
    async def handle_rupture(self, msg: NodeErrorMessage) -> None:
        ...
        RuntimeController.handle_rupture(self.name, self.transition, msg.source_node, msg.error)
...
def get_environment_context(args: argparse.Namespace) -> Tuple[dict, dict, Optional[SurgeBlueprint]]:
    ...
    if not args.prompt:
        ...
        # 💡 Manifest -> Registry를 거쳐 조립된 최종 Blueprint를 안정적으로 획득
    ...
    run_context = {
        ...
        "topology_spec": DEFAULT_TOPOS_SPEC,
        "entry_point": "ConfigPolicyNode"
    }
...
def main():
    ...
    # 💡 동적 Registry를 기반으로 안전하게 Choices 바인딩
    parser.add_argument("-s", "--scenario", type=str, choices=list(BLUEPRINT_REGISTRY.get(BlueprintType.SCHEME, {}).keys()))
    parser.add_argument("-t", "--transaction", type=str, choices=list(BLUEPRINT_REGISTRY.get(BlueprintType.TRANSACTION, {}).keys()))
