...
## @lineage: surgent.flow.align.commit
import argparse
import sys
import subprocess
import asyncio
...
# [수정] tag 파라미터 수신 및 물리적 태깅 수행 로직 추가
def git_commit_runner(path: Path, message: str, apply: bool, tag: str = None) -> str:
    ...
    try:
        ...
        if not status:
            ...
            # 변경사항이 없더라도 얼라인먼트를 위해 최신 커밋에 태그 강제 생성
            if tag:
                subprocess.run(["git", "tag", "-f", tag], cwd=path, check=True)
...
def entry_task(args):
    ...
    # [수정] --tag 인수 추가
    parser.add_argument("-t", "--tag", type=str, default=None, help="Global version tag to apply across all nodes (e.g., v1.0.0)")
