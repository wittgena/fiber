...
* **Cryptographic Adapter (부패 방지 계층):** 커넥션 기반 세션이 사라진 자리를 완벽히 대체합니다. 파편화된 암호화 스펙(Ed25519, RSA, EC)을 정규화하고, 매 요청마다 **DPoP 서명**과 **x402 결제 영수증**을 검증합니다. 분산 캐시 기반의 난수 락(`NonceReplayProtector`)을 통해 리플레이 공격과 중복 결제를 엣지 단에서 즉시 차단합니다.
...
* x402 결제를 승인하고 원장에 `PENDING` 상태 전이를 제안합니다.
...
* AI 에이전트의 연산은 정확한 WASM 인스트럭션 사이클(Instruction Cycle) 단위로 측정되어 **수학적으로 완벽한 x402 Fuel(연료) 과금 종량제**를 완성합니다.
...
| **402 Payment** | `-32002` | `KERNEL_AUTHORIZATION_REJECTED` | x402 스테이블코인 잔액 부족. |
