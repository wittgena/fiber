...
class ChainDiagnosticFlow:
    ...
    async def _setup_physical_membrane(self, tunnel):
        log.info("[Diag] Provisioning Real Physical Membrane (Control Plane & Worker)...")
        await KernelGateway.assemble(None)
        self._watcher_task = asyncio.create_task(receptor_bootstrap(tunnel))
        
        # [핵심 개선] 모듈화된 AutoScaler 의존성 주입 및 가동 (디버그 이벤트 포함)
    ...
    async def _inject_synthetic_load(self, tunnel):
        ...
        """
        log.info("\n[Diag] Injecting continuous synthetic high load trajectory...")
        counter = 0
        try:
            while not self.scale_out_success.is_set():
                # 0, 10, 40, 90, 160... 처럼 제곱으로 폭발시킴
    ...
    async def run(self):
        ...
        try:
            ...
            # 워커와 데몬이 켜질 물리적 시간을 3초 부여 (안정성 확보)
            log.info("[Diag] Waiting 3 seconds for all daemons and listeners to fully boot...")
            await asyncio.sleep(3.0)
            
            self._injector_task = asyncio.create_task(self._inject_synthetic_load(tunnel))
            
            try:
                await asyncio.wait_for(self.scale_out_success.wait(), timeout=10.0)
                log.info("\n🎉 [SUCCESS] PERFECT! The Real Physical Pub/Sub chain is connected.")
                log.info(f"🎉 Total running worker nodes: {len(self.worker_processes)}")
