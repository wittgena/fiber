...
from ext.router.adapter.constants import DEFAULT_EMBED_BATCH_SIZE
from ext.router.adapter.types.schema import BaseNode, MetadataMode, TransformComponent
from ext.router.adapter.parser import get_tqdm_iterable
from ext.router.adapter.parser import run_jobs

Embedding = List[float]

from ext.router.adapter.callback.event import EmbeddingEndEvent, EmbeddingStartEvent
...
class BaseEmbedding(TransformComponent):
    ...
    def check_base_embeddings_class(self) -> Self:
        from ext.router.adapter.storage.kvstore.types import BaseKVStore
