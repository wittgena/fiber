...
from ext.router.adapter.constants import DEFAULT_CONTEXT_WINDOW, DEFAULT_NUM_OUTPUTS
from ext.router.adapter.types.schema import ImageDocument
from ext.router.adapter.parser import get_tokenizer, resolve_binary
...
class BaseContentBlock(ABC, BaseModel):
    ...
    def get_template_vars(self) -> list[str]:
        ...
        """
        from ext.router.adapter.parser import get_template_vars
    ...
    def format_vars(self, **kwargs: Any) -> "BaseContentBlock":
        ...
        For that to work, the validation on those fields would need to be updated though.
        """
        from ext.router.adapter.parser import format_string
...
class TextBlock(BaseContentBlock):
    ...
    async def asplit(
        ...
        from ext.router.adapter.node_parser import TokenTextSplitter
