...
class FlarePipelineConfig:
    ...
    @desc: Registry of test scenes to validate in the edge environment.
           Reuses Wasm core semantics while verifying physical edge limits.
    """
    suites_registry: Dict[str, str] = field(default_factory=lambda: {
        # [수정됨] 3개의 씬을 하나로 통합한 Cloudflare Native 단일 파이프라인으로 매핑
        "flare": "fiber.workflow.scene.flare:FlareUnifiedScene",
    })
    
    default_suites: List[str] = field(default_factory=lambda: [
        # [수정됨] 기본 실행 씬을 단일 'flare' 씬으로 통일
        "flare",       ## Cloudflare Native Unified Validation Pipeline
    ])

class FlareFlow:
    """
    @role: Cloudflare Edge Pipeline Orchestrator
    @desc: Dynamically provisions the V8 Polyglot Sandbox via FlareController
           and executes end-to-end integration tests at the Edge.
    """
