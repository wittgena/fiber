# ator.driver.tool.fedit.executor
## @lineage: driver.tool.fedit.executor
## @lineage: ator.bound.tool.fedit.executor
## @lineage: eco.tool.fedit.executor
## @lineage: engine.tool.fedit.executor
## @lineage: phi.tool.fedit.executor
## @lineage: phi.runtime.tool.fedit.executor
## @lineage: swarm.mesh.tool.fedit.executor
...
from ator.driver.tool.fedit.editor import FileEditor
from ator.driver.tool.fedit.exceptions import ToolError

_GLOBAL_EDITOR: FileEditor | None = None

class FileEditorExecutor(ActionExecutor):
    def __init__(
        self,
    ...
    def __call__(
        ...
        context: "ToolExecutionContextProtocol | None" = None,
    ) -> FileEditorObservation:
...
def file_editor(
    ...
    try:
        ...
        result = FileEditorObservation.from_text(text=e.message, command=command, is_error=True)
    assert result is not None, "file_editor should always return a result"
    return result