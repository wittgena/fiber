# ator.state.event.llm.message
## @lineage: engine.atoa.event.llm.message
## @lineage: engine.protocol.atoa.event.llm.message
## @lineage: phi.agent.atoa.event.llm.message
## @lineage: agent.atoa.event.llm.message
import copy
...
from engine.parser.conv.event import N_CHAR_PREVIEW, EventID, LLMConvertibleEvent
from engine.parser.conv.event import SourceType
...
class MessageEvent(LLMConvertibleEvent):
    ...
    llm_response_id: EventID | None = Field(default=None, description="Completion or Response ID of the LLM response that generated this event",)
