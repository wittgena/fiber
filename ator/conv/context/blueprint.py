# ator.conv.context.blueprint
## @lineage: ator.context.blueprint
...
#         # =========================================================================
#         # [개선됨] 대문자 변환(upper)을 제거하고, 명시적인 소문자 툴 호출 지시로 변경
#         # 이 변경으로 인해 모델이 'TERMINAL'이라는 없는 툴을 호출하는 환각이 방지됩니다.
#         # =========================================================================
#         node = SurgeNode(
#             id=f"step_{i+1}_{action_name}",
#             intent=step.get("intent", "execute"),
...
#     # =========================================================================
#     # [개선됨] 필수 파라미터 강제 및 툴 이름 소문자 사용을 프롬프트 레벨에서 강력히 통제
#     # =========================================================================
#     system_instructions = (
#         f"You are executing the '{focus}' topology under {target_tier} constraints (Fuel Limit: {fuel_limit}).\n"
#         f"Navigate through the specified {len(steps)} events sequentially.\n"
...
#         f"If an anomaly is detected, trigger the 'signal' tool to broadcast architectural telemetry.\n\n"
#         f"CRITICAL RULES FOR FUNCTION CALLING:\n"
#         f"1. EXACT TOOL NAMES: You must strictly use the exact lowercase tool names as registered (e.g., '{CoreAction.TERMINAL.value}', '{CoreAction.SIGNAL.value}', '{CoreAction.FINISH.value}').\n"
#         f"2. REQUIRED PARAMETERS: Never omit required parameters. For example, when using the '{CoreAction.TERMINAL.value}' tool, you must provide BOTH 'command' and 'security_risk'."
#     )
