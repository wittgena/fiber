...
from fiber.agent.infra.observer.oracle.receptor import OracleReceptor
from fiber.agent.infra.bridge.protocol import AsyncAgentProtocol
    ...
    async def _execute_kline_fetch(self, req_id: Any, arguments: Dict[str, Any]):
        ...
        try:
            ...
            if cached_item and (now - cached_item['ts']) < self._cache_ttl:
                ...
                # [개선] Blocking I/O 발생 지점 제거
                # await를 통해 외부 네트워크를 다녀오는 동안 이 워커는 블로킹되지 않고 다른 요청을 동시 처리합니다.
                sealed_payload = await self.receptor.fetch_and_seal_async(
