# agent.resolver.account.openai
## @lineage: agent.topos.resolver.account.openai
## @lineage: actor.topos.resolver.account.openai
## @lineage: topos.resolver.account.openai
...
from mesh.bound.secure.cred import CredentialStore, OAuthCredentials, get_credentials_dir
...
class OpenAISubscriptionAuth:
    ...
    def create_llm(
        ...
        **llm_kwargs: Any,
        ...
        if "brane_extra_body" in llm_kwargs:
            extra_body.update(llm_kwargs.pop("brane_extra_body"))

        # Build headers matching OpenAI's official Codex CLI
        extra_headers: dict[str, str] = {
            "originator": "codex_cli_rs",
            "OpenAI-Beta": "responses=exp",
            "User-Agent": f"surgent ({platform.system()}; {platform.machine()})",
        }
        if account_id:
...
DEFAULT_SYSTEM_MESSAGE = ("You are Meta Debug Agent, a helpful AI assistant that can interact with a computer to solve tasks.",)
