...
from fiber.agent.topos.ator import Ator

from fiber.dphi.space.action.action import Action, Observation
from fiber.dphi.space.action.message import Message, MessageToolCall, ReasoningItemModel, RedactedThinkingBlock, TextContent, ThinkingBlock
from fiber.runtime.event.action import ActionEvent
from fiber.runtime.event.message import MessageEvent
from fiber.runtime.event.system import SystemPromptEvent, TokenEvent
from fiber.runtime.event.observation import ObservationEvent, UserRejectObservation, AgentErrorEvent
from fiber.runtime.action.parser import format_context_exceeded_message, ActionParser
from fiber.runtime.loop.step import StepHandler, StepContext
...
class Activator(Ator):
    ...
    def set_step_handlers(self, handlers: list[StepHandler], is_graph_mode: bool = False) -> None:
        """외부에서 조립된 Handler 파이프라인을 주입받는 유일한 통로"""
    ...
    # [핵심 변경] Organizer 마운트 로직을 완전히 제거하고, 
    # 오직 스키마를 파싱하여 dag_materials를 갱신하는 순수 컴파일 기능만 남깁니다.
    # 마운트는 이 함수를 호출한 외부(Executor 등)에서 수행해야 합니다.
