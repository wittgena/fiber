# fiber.agent.client.engine.protocol.terminal
## @lineage: surgent.topos.protocol.terminal
## @lineage: surgent.agent.protocol.terminal
## @lineage: surgent.agent.executor.terminal
## @lineage: surgent.space.terminal.executor
import json
import threading
import time
...
from fiber.agent.client.tool.schema.executor import ActionExecutor
from fiber.agent.client.tool.schema.terminal import TerminalAction, TerminalObservation
from fiber.agent.client.engine.protocol.context import ToolExecutionContextProtocol
from fiber.agent.client.tool.terminal.session import TerminalSession
from fiber.agent.client.tool.terminal.context import ExecutionContext, ExecutionEngine
from fiber.agent.client.tool.terminal.polling import PollingExecutionEngine

from fiber.agent.client.engine.protocol.builder import ChainBuilder
from fiber.agent.client.engine.protocol.builder import _is_tmux_available, create_terminal_session
from fiber.agent.client.engine.tmux.pool import DEFAULT_MAX_PANES, PooledTmuxTerminal, TmuxPanePool
...
class TerminalExecutor(ActionExecutor[TerminalAction, TerminalObservation]):
    ...
    def __init__(
        ...
        if use_pool:
            ...
            log.info(
                f"TerminalExecutor configured (pool mode, Lazy) "
                f"working_dir: {working_dir}, username: {username}, max_panes: {max_panes}"
            )
        else:
            ...
            log.info(
                f"TerminalExecutor configured (single mode, Lazy) "
    ...
    def _ensure_initialized(self) -> None:
        if self._pool is not None:
            ...
            if not getattr(self._session.terminal, "_initialized", False):
    ...
    def _reset_single_session(self) -> TerminalObservation:
        ## 단일 세션 리셋 시, 먼저 기존 것이 있는지 확인 후 닫고 새로 만듭니다.
