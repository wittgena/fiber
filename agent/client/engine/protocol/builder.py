# fiber.agent.client.engine.protocol.builder
## @lineage: surgent.topos.protocol.builder
## @lineage: surgent.agent.protocol.builder
## @lineage: surgent.agent.executor.builder
## @lineage: surgent.space.terminal.session.builder
## @lineage: space.terminal.session.builder
...
import platform
import warnings
import functools  # [개선됨] 캐싱을 위해 추가
...
from fiber.agent.client.event.observation import AgentErrorEvent
from fiber.agent.client.tool.schema.terminal import TerminalObservation
from fiber.agent.client.tool.terminal.context import (
    ExecutionContext,
    ExecutionEngine,
    ExecutionMiddleware,
)
...
if TYPE_CHECKING:
    ...
    from fiber.agent.client.tool.schema.builder import DeclaredResources, ActionDefinition
...
# [개선됨] 캐싱을 적용하여 최초 1회만 서브프로세스를 생성하고 이후엔 캐시된 결과를 즉시 반환
...
def create_terminal_session(
    ...
    if terminal_type:
        ...
        if terminal_type == "tmux":
            ...
            from fiber.agent.client.tool.terminal.backend import SubprocessTerminal
