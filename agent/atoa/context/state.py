# agent.atoa.context.state
## @lineage: agent.conver.conv.state
## @lineage: phi.conver.conv.state
## @lineage: swarm.conver.conv.state
import os
import json
import inspect
...
from agent.atoa.event.llm.observation import (
    AgentErrorEvent,
    ObservationEvent,
...
from agent.atoa.conv.types import ConversationCallbackType, ConversationID, ConversationTags

from phi.security.confirm import ConfirmationPolicyBase, NeverConfirm
...
from agent.atoa.context.io import IOManager
from topos.state.store.log import LogStore, VirtualEventLogProxy
from topos.state.command import (
    StateCommand, 
    TransitionStatus, 
    BlockAction, 
    BlockMessage, 
    ...
    def __setattr__(self, name, value):
        ...
        if old is _sentinel or old != value:
            ...
            if callback is not None and old is not _sentinel:
                try:
                    from agent.atoa.event.conv import ConversationStateUpdateEvent
