# agent.atoa.disc.memory.codon.registry
## @lineage: atoa.agent.disc.memory.codon.registry
...
if TYPE_CHECKING:
    from agent.topos.activator import Activator
...
class AtorDefinition(BaseModel):
    ...
    def get_confirmation_policy(self) -> ConfirmationPolicyBase | None:
        ...
        if self.permission_mode == "always_confirm":
            from agent.gov.security.confirm import AlwaysConfirm
        ...
        if self.permission_mode == "confirm_risky":
            from agent.gov.security.confirm import ConfirmRisky
            return ConfirmRisky()
