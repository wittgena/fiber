# agent.atoa.driver.tensor
## @lineage: atoa.agent.driver.tensor
...
from agent.eco.secure.secret.validator import serialize_secret, validate_secret

from agent.gov.exception.types import LLMNoResponseError
from agent.atoa.disc.memory.profile import LLMProfileStore
from agent.atoa.retry import RetryMixin
from agent.atoa.mock.mixin import MockToolCallMixin
from agent.gov.exception.types import LLMContextWindowTooSmallError
from agent.gov.exception.mapping import map_provider_exception
from agent.atoa.response import LLMResponse
from agent.atoa.types import TokenCallbackType
from agent.atoa.fallback import FallbackStrategy
from agent.atoa.driver.io import DriverIO
from agent.atoa.driver.factory import DriverFactory

from agent.atoa.action.definition import ActionDefinition
from agent.eco.action.message import Message
from agent.atoa.config.vendor import VendorSubstrateMixin
from agent.eco.residue.convset import SettingProminence, field_meta
from agent.eco.residue.depre import warn_deprecated
...
class Driver(VendorSubstrateMixin, RetryMixin, MockToolCallMixin):
    """
    @desc: Tensor Substrate Driver
    @role: Resolves external LLM topologies, encapsulating parameter projection and resilient I/O.
           Vendor-specific parameters are seamlessly injected via VendorSubstrateMixin.
           Observability is securely delegated to a Stateless DriverObserver.
    """
    model: str = Field(
