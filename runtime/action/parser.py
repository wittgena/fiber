...
## @lineage: agent.loop.conv.action.parser
## @lineage: agent.runtime.conv.action.parser
...
from fiber.dphi.space.action.builder import ActionDefinition
from fiber.runtime.event.action import ActionEvent
from fiber.runtime.event.observation import AgentErrorEvent
...
from fiber.dphi.space.action.message import (
    MessageToolCall,
    ReasoningItemModel,
...
class ActionParser:
    ...
    def parse_tool_call(
        ...
        # [수정됨] Pydantic 에러 방지를 위해 Enum 객체 대신 순수 문자열(String)로 초기화합니다.
        # ActionEvent 생성 시 Pydantic이 이 문자열을 다시 내부 Enum으로 자동 파싱합니다.
        default_risk_val = (
            risk.SecurityRisk.UNKNOWN.value 
            if hasattr(risk.SecurityRisk.UNKNOWN, 'value') 
            else "UNKNOWN"
        )
        security_risk: str = default_risk_val
        ...
        action_event = ActionEvent(
            ...
            llm_response_id=llm_response_id,
            security_risk=security_risk,  # 이제 Enum 객체가 아닌 안전한 문자열(str)이 주입됩니다.
            summary=summary,
        )
