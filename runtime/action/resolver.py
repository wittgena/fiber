...
## @lineage: agent.loop.conv.action.resolver
## @lineage: agent.runtime.conv.action.resolver
## @lineage: ator.runtime.action.resolver
## @lineage: agent.bridge.action.resolver
import json
...
from fiber.runtime.action.factory import MessageIntent, TopologicalIntent, CoreAction, ActionProxy, build_action
...
def _handle_bridge(action: Any, conv: "ProtoConv | None", ObsClass: type[Observation]) -> Observation:
    from xphi.kernel.space.topos.context.status import ConverStatus
    from fiber.runtime.event.observation import ObservationEvent
    from xphi.arch.contract.event.next import next_id
    from fiber.runtime.command.conv import TransitionStatus, UpdateAgentState
